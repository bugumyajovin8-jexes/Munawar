...
const { needsMore, nextSize, burnRate, take, remaining, blockForYear, pruneBlocks,
  shouldCarryNextYear, COLD_START_SIZE, MAX_SIZE, MIN_SIZE } = plan;
...
check(
  "next year is carried from December",
  shouldCarryNextYear(new Date("2026-12-01T09:00:00Z")) === true,
  "December",
);
check(
  "November is too early to bother",
  shouldCarryNextYear(new Date("2026-11-30T09:00:00Z")) === false,
  "November",
);

{
  const held = [block(), block({ year: 2027, start: 1, end: 50, next: 1 })];
  check(
    "the right year is spent, not merely the first held",
    blockForYear(held, 2027)?.start === 1 && blockForYear(held, 2026)?.start === 100,
    "two years held",
  );
}

// A device offline across New Year must still be able to issue.
{
  const held = [block(), block({ year: 2027, start: 1, end: 50, next: 1 })];
  const taken = take(blockForYear(held, 2027), 2027);
  check("issuing works the day the year turns", taken?.number === 1, taken?.number);
}

{
  const kept = pruneBlocks(
    [block({ year: 2025 }), block({ year: 2026 }), block({ year: 2027 })],
    2026,
  );
  check(
    "last year's block is dropped, this year's and next kept",
    kept.length === 2 && kept.every((b) => b.year >= 2026),
    kept.map((b) => b.year).join(","),
  );
}
