...
export async function storageReport() {
  ...
  console.info(
    ...
    // The logo's navy, same as the status bar — see manifest.ts.
    "background:#09142f;color:#fff;padding:2px 6px;border-radius:4px;font-weight:600",
  ...
  console.info(
    "Try: munawar.storage(), munawar.queue(), munawar.caches(), munawar.resync()",
  );
  console.groupEnd();
...
export function installDiagnostics() {
  ...
  Object.defineProperty(window, "munawar", {
    value: Object.freeze({
      ...
      requestPersistence,
      /**
       * Throw away the sync cursor and download the business again.
       *
       * The escape hatch for a mirror that has gone quietly out of step —
       * usually because a table was unreadable for a while and the cursor
       * moved past its rows. Safe to run at any time: it re-reads, it does not
       * delete, and nothing queued is touched.
       */
      resync,
    }),
    writable: false,
