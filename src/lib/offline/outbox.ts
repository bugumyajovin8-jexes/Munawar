...
/**
 * How long to wait after the server says it could not cope right now.
 *
 * A deferred operation is one nothing is wrong with — the database timed out,
 * the connection dropped, the function cold-started past its limit. Retrying
 * instantly is the worst possible response: the queue drains in a tight loop
 * against a server that is already struggling, which is how a slow minute
 * becomes an outage. Backing off doubles up to a ceiling and resets the
 * moment anything succeeds.
 */
const DEFER_BASE_MS = 5_000;
const DEFER_MAX_MS = 5 * 60_000;
...
/** Set when the server asks us to come back later. See DEFER_BASE_MS. */
let deferUntil = 0;
let deferStreak = 0;
...
/**
 * Loading the queue off the device, exactly once.
 *
 * The promise is what is shared, not a boolean. A flag set before the await
 * let a second caller through while `readAll()` was still in flight: it
 * appended its item to an empty in-memory queue, and the load then landed on
 * top and threw it away. The write survived in IndexedDB — it was not lost —
 * but it did not send, and it did not appear in the pending list, so the one
 * thing the user had to go on said nothing had happened.
 */
...
export async function submit(
  ...
  try {
    ...
    if (missed && getOnline()) await flush(true);
  } finally {
...
/**
 * `force` skips the backoff, and only a deliberate action should pass it.
 *
 * Somebody pressing "record payment" deserves an attempt now — they are stood
 * in front of the customer. A timer firing during a backoff window does not,
 * because backing off is the entire point.
 */
export function flush(force = false): Promise<void> {
  if (inFlight) return inFlight;
  if (force) deferUntil = 0;
...
async function drain(): Promise<void> {
  ...
  try {
    ...
    for (const result of results) {
      ...
      if (result.status === "rejected" || result.status === "deferred") {
        // Deferred is not blocked. Nothing is wrong with the operation, so it
        // keeps its place and goes again — after a pause, so a struggling
        // server is not hammered by every device that has work waiting.
        if (result.status === "rejected") blockedIds.add(result.id);
        else deferred = true;
    ...
    if (deferred) {
      deferStreak += 1;
      deferUntil =
        Date.now() + Math.min(DEFER_BASE_MS * 2 ** (deferStreak - 1), DEFER_MAX_MS);
    } else {
      deferStreak = 0;
      deferUntil = 0;
    }
  ...
  // More waiting than one batch could carry. The backoff is checked first —
  // without it a deferred batch would re-enter here immediately and spin
  // against the very server that just asked for a pause.
  if (
    Date.now() >= deferUntil &&
    mine().some((item) => !blockedIds.has(item.id)) &&
    getOnline()
  ) {
    await drain();
  }
