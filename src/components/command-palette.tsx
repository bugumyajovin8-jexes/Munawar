...
import { searchLocal, type SearchHit } from "@/lib/offline/search-local";
...
export function CommandPalette({ role }: { role: UserRole }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      /*
       * Reading the device, not the server, and catching regardless.
       *
       * This was a server action awaited here with nothing around it. Offline
       * the rejection was unhandled, setResult never ran, and `loading` —
       * derived from "the result does not match the query yet" — stayed true
       * for good. A spinner that never stops, on a control that is one
       * keystroke away from every screen in the app.
       *
       * An empty result set is now the honest answer to a failed read, and it
       * is one the interface already knows how to say.
       */
      let results: SearchHit[] = [];
      try {
        results = await searchLocal(trimmed);
      } catch {
        // Nothing readable on this device. "No matches" beats a spinner.
      }
      ...
      setCursor(0);
      // 120ms rather than 200: this is an IndexedDB read now, not a round trip,
      // so the debounce only has to outrun typing.
    }, 120);
