...
export function DashboardBody() {
  ...
  /*
   * Only this month's lines, by index, rather than every line ever written.
   *
   * invoice_items is the biggest table on the device — several rows per
   * invoice — and reading all of it to answer a question about one month was
   * the heaviest thing this screen did, repeated on every change to the
   * mirror.
   */
  const monthInvoiceIds = useMemo(
    () =>
      invoicesRaw
        .filter((i) => i.status === "issued" && i.invoice_date?.startsWith(month))
        .map((i) => i.id),
    [invoicesRaw, month],
  );
  ...
  const view = useMemo(() => {
    ...
    // Says why when it cannot be computed, rather than reporting a zero the
    // owner would read as a month spent selling at cost.
    const profitThisMonth = grossProfit(
      items,
      new Set(monthInvoices.map((i) => i.id)),
      // A whole-invoice discount is on no line, so line margins alone would
      // report the month as more profitable than it was.
      new Map(monthInvoices.map((i) => [i.id, Number(i.discount_amount ?? 0)])),
    );
  ...
  }, [invoicesRaw, customers, payments, items, month, today]);
  ...
  return (
    <>
      <PageHeader
        ...
        description={`${session?.orgName ?? ""} · ${formatDate(today)}`}
        actions={
          <Button asChild>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatTile
          ...
          value={
            !isAdmin
              ? formatTZS(receivedThisMonth)
              : profitThisMonth.value === null
                ? "—"
                : formatTZS(profitThisMonth.value)
          }
          tone={!isAdmin || profitThisMonth.value !== null ? "success" : "default"}
          hint={
            !isAdmin
              ? undefined
              : profitThisMonth.value !== null
                ? `${formatTZS(receivedThisMonth)} received`
                : profitThisMonth.reason === "no-invoices"
                  ? "No invoices issued yet this month"
                  : profitThisMonth.reason === "not-synced"
