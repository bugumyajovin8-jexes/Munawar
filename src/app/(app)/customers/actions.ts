...
export async function saveCustomer(formData: FormData): Promise<FormState> {
  ...
  /*
   * An empty field is no discount, which is why this is not just clamped.
   * 100 or more would be an invoice for nothing, and saying so is more use
   * than storing a default that fails every time it is used.
   */
  const discount = Number(String(formData.get("default_discount_percent") ?? "").trim() || 0);
  if (!Number.isFinite(discount) || discount < 0 || discount >= 100) {
    return { error: "A discount must be between 0 and 99.99 percent.", ok: false };
  }
  ...
  const row = {
    ...
    default_discount_percent: Math.round(discount * 100) / 100,
...
/**
 * Create or edit one of a customer's branches.
 *
 * Shaped like saveCustomer deliberately, down to the client-minted id: a
 * branch added with no signal has to be usable on an invoice raised a minute
 * later, still with no signal, and that is only possible if the device chose
 * the id rather than waiting for Postgres to mint one.
 *
 * Retiring one is a matter of clearing is_active, not deleting. A branch with
 * invoices against it cannot be deleted at all — the reference is ON DELETE
 * RESTRICT, because a document naming a place that has vanished from the
 * record is worse than an untidy list.
 */
export async function saveBranch(formData: FormData): Promise<FormState> {
  const session = await requireSession();
  const supabase = await createClient();
  ...
  if (!customerId) return { error: "A branch must belong to a customer.", ok: false };
  if (!name) return { error: "Give the branch a name.", ok: false };
  ...
  const insertRow: typeof row & { id?: string } = clientId ? { ...row, id: clientId } : row;

  const { error } = id
    ? await supabase.from("customer_branches").update(row).eq("id", id)
    : await supabase.from("customer_branches").insert(insertRow);

  if (error) {
    /*
     * The unique index is on (customer_id, lower(trim(name))), so this means
     * somebody is adding a second "Mwanza". Saying that is more use than the
     * constraint name, which is all Postgres offers.
     */
    if (error.code === "23505") {
      return { error: `${name} is already a branch of this customer.`, ok: false };
    }
